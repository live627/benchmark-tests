...
class UltimateMenuController extends Action_Controller
{
	...
	public function action_savebutton()
	{
		...
		if (isset($_POST['submit']))
		{
			...
			{
				$context['page_title'] = empty($menu_entry['id'])
					? $txt['um_menu_add_title']
					: $txt['um_menu_edit_title'];
				...
				$context['button_data']['all_permissions'] = empty(array_diff_key(
					$context['button_data']['permissions'],
					array_filter(
						array_column($context['button_data']['permissions'], 'checked', 'id')
					)
				));
			}
		}
