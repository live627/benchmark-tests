 ...
 */
class MyUuid implements \Stringable
{
	/*****************
	 * Class constants
	 *****************/
	 ...
	 * @var bool
	 */
	protected static bool $override_constructor = false;
	...
	public function __construct(
		...
		$this->hex = str_replace('-', '', $this->uuid);
	}
	...
	{
		if ($lowercase_only) {
			return self::encodeBase32Hex($this->hex);
		}
	 ...
	 */
	public static function createFromString(\Stringable|string $input, bool $strict = false): self
	{
		...
		if ($length === 16) {
			$hex = bin2hex($input);
		} elseif ($length === 22 && strspn($input, self::BASE64_SORTABLE) === 22) {
			$hex = bin2hex(base64_decode(strtr($input, self::BASE64_SORTABLE, self::BASE64_STANDARD), true));
		} elseif ($length === 26) {
			$input = strtolower($input);

			if (strspn($input, self::BASE32_ALT) === 26) {
				$hex = self::decodeBase32Hex($input);
		...
		if (!isset($hex)) {
			...
			if (\strlen($input) === 32 && strspn($input, '0123456789abcdef') === 32) {
				$hex = $input;
			} elseif ($strict) {
		...
		if ($variant === 1 && !\in_array($version, self::KNOWN_VERSIONS, true)) {
			if ($strict) {
				throw new \ValueError("Invalid UUID string supplied: {$input}");
			}
		...
		self::$override_constructor = true;
		$obj = new self();
		self::$override_constructor  = false;
		$obj->version = $version;
		$obj->hex = $hex;
		$obj->uuid =
			substr($hex, 0, 8) . '-' .
	...
	protected function getHexV2(array $input): string
	{
		...
		self::$node = \sprintf('%012x', hexdec(bin2hex(fake_random_bytes(6))) | 0x10000000000);
	...
	{
		return bin2hex(fake_random_bytes(16));
	}
...
protected function getHexV7(string|int|float|null $input): string
{
	...
	/******************************************************************************
	 * Optimized fixed-width hex (replaces sprintf)
	 ******************************************************************************/

	$hexTimestamp = dechex($timestamp);

	// manual left-pad to 12 chars (faster than sprintf)
	$hexTimestamp = str_pad($hexTimestamp, 12, '0', STR_PAD_LEFT);

	/******************************************************************************
	 * Random suffix (74 bits = 10 bytes)
	 ******************************************************************************/

	return $hexTimestamp . bin2hex(fake_random_bytes(10));
}
	...
	protected function getGregTimeParts(): array
	{
		...
		if (!isset(self::$clock_seq[$this->version])) {
			self::$clock_seq[$this->version] = bin2hex(fake_random_bytes(2));
		}
...
protected static function decodeBase32Hex(string $b32): string
{
	...
	for ($i = 0; $i < 26; $i++) {
		...
		// Translate ASCII to Crockford Base32.
		$value = match (true) {
			$c <= 57 => $c - 48,
			$c <= 104 => $c - 87,
			$c <= 107 => $c - 88,
